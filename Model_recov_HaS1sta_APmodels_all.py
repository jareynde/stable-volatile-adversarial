...
Model recovery of 4 models in stable context,
...
#Model 1
def RW_eps_lr_stable(eps, lr, T, Q_int):
    #eps        --->        epsilon
    #lr         --->        learning rate
...
#Model 2:
def RW_eps_lr_uc_stable(eps, lr, uc, T, Q_int):
    #eps        --->        epsilon
    #lr         --->        learning rate
...
#Model 5:
def RW_eps_2lr_stable(eps, lrpos, lrneg, T, Q_int):
    #eps        --->        epsilon
    #lr         --->        learning rate
    #T          --->        amount of trials for each simulation
    #Q_int      --->        parameter that determines initial Q value: intial Q values for all actions are equal to Q_int*(1/K)
...
#Model 6:
def RW_eps_2lr_uc_stable(eps, lrpos, lrneg, uc, T, Q_int):
    #eps        --->        epsilon
    #lr         --->        learning rate
    #uc         --->        unchosen value-bias
    #T          --->        amount of trials for each simulation
...
for strategy in strategies:
    ...
    #########################################################################################
    sample_dir = os.path.join(next_dir2, '1_HaS1sta_modelfit_eps_lr.xlsx')
    ...
    for i in range(recov_amount):
        ...
        while True:
            ...
            true_eps = random.uniform(0, 0.5)#float(sample_pool['eps'].iloc[ri])
            if true_eps < 0.5:
                break
        ri = np.random.randint(0, len(sample_pool['eps']))
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M1_modelrecov_eps_lr_sampled.xlsx')
        M1sampled_eps_lr.to_excel(title_excel, index=False)

        k, r, Q_k_stored = RW_eps_lr_stable(eps=true_eps, lr=true_lr, T=T, Q_int=Q_int)
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M1_1modelrecov_eps_lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M1_2modelrecov_eps_lr_uc.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M1_5modelrecov_eps_2lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M1_6modelrecov_eps_2lr_uc.xlsx')
        M1eps_2lr_uc.to_excel(title_excel, index=False)


        title_excel = os.path.join(save_dir, f'HaS1sta_M1_modelrecov_BIC_all.xlsx')
    ...
    #########################################################################################
    sample_dir = os.path.join(next_dir2, '2_HaS1sta_modelfit_eps_lr_uc.xlsx')
    sample_pool = pd.read_excel(sample_dir)
    ...
    for i in range(recov_amount):
        ...
        while True:
            ri = np.random.randint(0, len(sample_pool['eps']))
            true_eps = random.uniform(0, 0.5)#random.uniform(0, 0.5)#float(sample_pool['eps'].iloc[ri])
            if true_eps < 0.5:
                break
        ri = np.random.randint(0, len(sample_pool['eps']))
        true_lr = float(sample_pool['lr'].iloc[ri])
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M2_modelrecov_eps_lr_uc_sampled.xlsx')
        M2sampled_eps_lr_uc.to_excel(title_excel, index=False)

        k, r, Q_k_stored = RW_eps_lr_uc_stable(eps=true_eps, lr=true_lr, uc= true_uc, T=T, Q_int=Q_int)
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M2_1modelrecov_eps_lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M2_2modelrecov_eps_lr_uc.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M2_5modelrecov_eps_2lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M2_6modelrecov_eps_2lr_uc.xlsx')
        M2eps_2lr_uc.to_excel(title_excel, index=False)


        title_excel = os.path.join(save_dir, f'HaS1sta_M2_modelrecov_BIC_all.xlsx')
    ...
    #########################################################################################
    sample_dir = os.path.join(next_dir2, '5_HaS1sta_modelfit_eps_2lr.xlsx')
    sample_pool = pd.read_excel(sample_dir)
    ...
    for i in range(recov_amount):
        ...
        while True:
            ...
            if true_eps < 0.5 and abs(true_lrpos - true_lrneg) > 0.3:
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M5_modelrecov_eps_2lr_sampled.xlsx')
        M5sampled_eps_2lr.to_excel(title_excel, index=False)    

        k, r, Q_k_stored = RW_eps_2lr_stable(eps=true_eps, lrpos=true_lrpos, lrneg=true_lrneg, T=T, Q_int=Q_int)
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M5_1modelrecov_eps_lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M5_2modelrecov_eps_lr_uc.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M5_5modelrecov_eps_2lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M5_6modelrecov_eps_2lr_uc.xlsx')
        M5eps_2lr_uc.to_excel(title_excel, index=False)



        title_excel = os.path.join(save_dir, f'HaS1sta_M5_modelrecov_BIC_all.xlsx')
    ...
    #########################################################################################
    sample_dir = os.path.join(next_dir2, '6_HaS1sta_modelfit_eps_2lr_uc.xlsx')
    sample_pool = pd.read_excel(sample_dir)
    ...
    for i in range(recov_amount):
        ...
        k, r, Q_k_stored = RW_eps_2lr_uc_stable(eps=true_eps, lrpos=true_lrpos, lrneg=true_lrneg, uc=true_uc, T=T, Q_int=Q_int)
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M6_1modelrecov_eps_lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M6_2modelrecov_eps_lr_uc.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M6_5modelrecov_eps_2lr.xlsx')
        ...
        title_excel = os.path.join(save_dir, f'HaS1sta_M6_6modelrecov_eps_2lr_uc.xlsx')
        M6eps_2lr_uc.to_excel(title_excel, index=False)



        title_excel = os.path.join(save_dir, f'HaS1sta_M6_modelrecov_BIC_all.xlsx')
